...
def txt2tex2(tex_file):
    txt_file = 'aux.txt'
    # if(re.search('.txt$',txt_file)==None):
    #     sys.exit('The input should be .txt file. Exit.')

    # print('Input file:',txt_file)
    ...
    trtext=re.sub(r'\[111\]', '[1.11]', trtext) # speical case for baidu translate
    ...
    if(corrupted==[]):
        ...
        for c in corrupted:
            print(c,end=' ')
        print()
        print('To improve the output manually change the corrupted tokens in file',txt_file,'and run from.py again.')

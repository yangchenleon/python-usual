...
def read_time():
    filename = ".\\prediction-plot\\data\\20-100mm的时序预测\\time.xlsx"
    workbook = xlrd.open_workbook(filename)
    sheet = workbook.sheet_by_index(0)
    times_p = sheet.col_values(0)
    times_f = sheet.col_values(1)
    # print(times_f[1:], times_p[1:])
    return times_f[1:], times_p[1:]
...
def read_sheet_data(sheet):
    facts = sheet.col_values(1)
    preds = sheet.col_values(0)

    title = facts[0]
    
    return facts[1:], preds[1:], title
...
def draw_sheet(sheet, output_dir):
    ...
    t1, t2 = np.array(time[0]), np.array(time[1][:num_pred])
    title = data[2]
...
if __name__ == '__main__':
    # 没啥区别，1是时间和数据结合，2是分离读取，然后输入输出文件区别
    base_dir = ".\\prediction-plot"
    sub_dir = ["10mm", "40mm", "100mm", "40mm-2"]
    for dir in sub_dir:
        filename = f"{base_dir}\\data\\20-100mm的时序预测\\{dir}.xlsx"
        output_dir = f"{base_dir}\\svg-result\\{dir}的时序预测"
